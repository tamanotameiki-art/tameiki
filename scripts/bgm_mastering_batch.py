...
BGMシートの「マスタリング待ち」を自動検出して順番に処理する。
...
def get_sheet_service(creds_json_str):
...
def get_pending_bgms(spreadsheet_id, creds_json_str):
    ...
    sheets, _ = get_sheet_service(creds_json_str)
    ...
    for i, row in enumerate(rows[1:], start=2):
        ...
        if status == "マスタリング待ち" and file_id:
            pending.append({"row": i, "file_name": file_name, "file_id": file_id})
    return pending


def mark_status(spreadsheet_id, creds_json_str, row, status):
    """L列のステータスを更新（処理中フラグ・エラー戻し用）"""
    sheets, _ = get_sheet_service(creds_json_str)
...
def main():
    ...
    for target in targets:
        ...
        # 処理中フラグ（二重実行防止）
        mark_status(spreadsheet_id, creds_json_str, row, "処理中")
        ...
        try:
            ...
            subprocess.run(
                ["ffmpeg", "-y", "-i", current, "-af", eq_filters, eq_path],
            ...
            subprocess.run(
                ["ffmpeg", "-y", "-i", current, "-af", comp_filter, comp_path],
                capture_output=True, check=True
            )
            current = comp_path
            ...
            # SHEET_ROW を環境変数にセット（update_bgm_sheet が参照）
            os.environ["SHEET_ROW"] = str(row)
            preview_url, mastered_file_id = upload_mastered_to_drive(
                mastered_path, clip_paths, file_name
            )
            update_bgm_sheet(str(row), analysis, clips, preview_url, mastered_file_id)
            ...
            # エラー時はマスタリング待ちに戻す
            try:
                mark_status(spreadsheet_id, creds_json_str, row, "マスタリング待ち")
            except Exception:
                pass
