...
- 管理画面: https://tamanotameiki-art.github.io/tameiki/admin_tK9mPq2vXr8n.html
...
### URL（難読化済み・ブックマーク必須）
https://tamanotameiki-art.github.io/tameiki/admin_tK9mPq2vXr8n.html

### ファイルの場所
docs/admin_tK9mPq2vXr8n.html

### アクセス制限の方針
- GitHubリポジトリはPublicだがURLを知らなければ辿り着けない難読化方式を採用
- URLはどこにも公開しない・SNSや公式サイトにリンクしない
- 必要に応じてファイル名を変更してURLを更新する

### 管理画面で管理できること
- ホーム：次回投稿予定の詩プレビュー・KPI（詩在庫/動画素材/再生数/投稿数）・直近7投稿の再生数グラフ・投稿履歴テーブル
- パフォーマンス：フィルター別YouTube平均再生数・Instagram累計・YouTube累計
- ワークフロー：4つのGitHub Actionsの稼働状況・未解決エラー一覧・手動実行ボタン
- 詩管理：未投稿/投稿済み/合計の在庫確認・詩一覧テーブル・在庫10本以下で警告表示
- 動画素材管理：有効/無効/合計の確認・素材一覧テーブル・有効数10本以下で警告表示
- BGM管理：マスタリング状況・フルMV生成状況・使用回数
- 設定：投稿時刻・フィルター固定・各SNS投稿ON/OFF・クイックリンク集
- ログ：実行ログ（現在はダミー表示・GitHub Actionsで詳細確認）
- AIチャット（クロちゃん）：Claude APIに接続・システムについて質問できる

### データソース
- スプレッドシートのCSV公開URLからデータを取得（認証不要）
- 手動実行はGitHub Personal Access Tokenが必要（初回入力でlocalStorageに保存）
- スプレッドシートのGIDは現在仮設定。実際のGIDに合わせて要更新。

### 今後の改善予定
- スプレッドシートの各シートGIDを実際の値に修正（現在:投稿履歴=763442013・他シートは要確認）
- ダッシュボードシート作成後にパフォーマンスページをより充実させる
- ログをGitHub Actions APIから実際に取得する（現在はダミー）
- 認証方式の見直し（Cloudflare Access等）が必要になった場合は対応
...
### 管理画面のGID設定
admin_tK9mPq2vXr8n.htmlのJavaScript内GIDが仮設定のまま。
実際のスプレッドシートの各シートGIDを確認して修正が必要。
投稿履歴シートのGIDは763442013（CSV公開URLより確認済み）。
文字列・動画素材・BGMシートのGIDは要確認。
...
### 9. 管理画面 — スプレッドシートGID修正
状況：admin_tK9mPq2vXr8n.html内のGIDが仮設定のまま。
やること：スプレッドシートで各シートタブを開きURLのgid=XXXXXXXXXを確認して、
admin_tK9mPq2vXr8n.htmlのGID定数を修正してpushする。